...
import { saveAsTxtWithPicker, openFile, triggerFileSelect } from './file-handler.js';
...
async function handleSave() {
    ...
    const defaultFilename = state.currentFilename.replace(/\.[^/.]+$/, '') + '.txt';

    try {
        const fileHandle = await saveAsTxtWithPicker(content, defaultFilename);
        if (fileHandle) {
            console.log(`已儲存檔案：${fileHandle.name}`);
